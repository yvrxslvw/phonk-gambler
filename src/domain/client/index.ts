...
class Bot extends Client {
	...
	public loginClient = async () => {
		try {
			...
			console.log(
				greenBright(`${blueBright(this.user?.tag!)} has been logged. ${yellowBright(`(${getCurrentTime(runtime)}s)`)}`),
			);
		} catch (error) {
			throw error;
		}
	...
	public putCommands = async (commands: Command[]) => {
		try {
			const runtime = performance.now();
			await this.rest.put(Routes.applicationGuildCommands(this.APP_ID, this.GUILD_ID), { body: commands });
			console.log(
				greenBright(
